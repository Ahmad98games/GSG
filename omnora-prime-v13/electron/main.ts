...
if (!gotTheLock) {
   ...
   */
  function waitForNextJS(port: number, maxAttempts = 60): Promise<void> {
  ...
  async function createWindow(): Promise<void> {
    ...
    if (isDev) {
      ...
      await mainWindow.loadURL(url).catch((e) =>
        startupLog(`[Electron] Dev loadURL error: ${e.message}`)
      );
  ...
  app.whenReady().then(async () => {
    ...
    if (!isDev) {
      ...
      try {
        ...
        dialog.showErrorBox(
          'Startup Timeout',
          `The app server did not start on port ${PORT}.\n\nCheck logs at:\n${logPath}`
        );
        app.quit();
        return;
      }
