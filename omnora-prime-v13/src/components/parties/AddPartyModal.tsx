...
export function AddPartyModal({
  ...
  const [partyType, setPartyType] =
    useState<PartyType>(defaultType || 'customer')
  ...
  const [openingBalance, setOpeningBalance] =
    useState('')
  const [balanceNature, setBalanceNature] =
    useState<BalanceNature>('receivable')
  ...
  const handleSubmit = useCallback(
    ...
    if (atLimit('parties' as any)) {
      toast.error(
        'Party limit reached on Free plan. Upgrade to add more.'
      )
      return
    }
    ...
    try {
      ...
      // Post opening balance ledger entry
      if (balanceAmount > 0) {
        await supabase.from('ledger_entries')
          .insert({
            ...
            debit: balanceNature === 'receivable'
              ? balanceAmount : 0,
            credit: balanceNature === 'payable'
              ? balanceAmount : 0,
            reference: 'OB',
          })
      }

      // Increment party count in license store
    ...
    partyType, openingBalance, balanceNature,
    creditLimit, creditTerms, profile, atLimit, onSuccess, onClose, defaultType, toast])
        ...
        <div className="px-6 py-5
            ...
            <div className="grid grid-cols-3
              ...
              {PARTY_TYPES.map(type => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setPartyType(type.value)}
                  className={`
                    py-2.5 px-2 rounded-sm
                    border text-center
                    transition-all duration-150
                    ${partyType === type.value
                      ? 'bg-[#60A5FA]/10 border-[#60A5FA]/40 text-[#60A5FA]'
                      : 'bg-[#0F1114] border-white/8 text-gray-500 hover:border-white/15 hover:text-gray-300'}
                  `}
                >
                  <p className="text-[10px]
          ...
          <div>
            ...
            <input
              ...
              placeholder="Al-Hameed Textile Mills"
            ...
            {showBalanceNature ? (
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setBalanceNature(
                      e.target.value as BalanceNature
                    )}
                  className="noxis-input"
                >
                  <option value="receivable">
                    Receivable (Jama) — They owe
                  </option>
          ...
          <div className="p-3 rounded-sm
            ...
            <div className="w-4 h-4 rounded
              bg-[#60A5FA]/20 flex items-center
            ...
            <p className="text-[10px]
              ...
              sub-ledger context. Credit
              limit or credit term breaches
              will trigger automatic
