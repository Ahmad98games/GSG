...
import { Check, X, ShieldCheck, Zap, Star, Smartphone, Building2, CreditCard, Globe } from "lucide-react";
import { motion } from "framer-motion";
import { useBusinessProfile } from "@/hooks/useBusinessProfile";
import { formatCurrency, CURRENCIES } from "@/lib/currency/currencyEngine";
...
function PricingCard({ 
  ...
  return (
    <div className={cn(
      ...
      popular ? "bg-[#121417] border-electric-blue shadow-[0_30px_60px_rgba(0,112,243,0.1)] scale-105 z-10" : "bg-[#0F1113] border-white/5 hover:border-white/10"
    )}>
      {popular && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-electric-blue text-onyx text-[9px] font-black uppercase tracking-[0.3em] px-6 py-2">
           Industrial Standard
        </div>
      )}
      ...
      <div className="flex-1 space-y-4 mb-12">
        {features.map((f: any, i: number) => (
          <div key={i} className={cn("flex items-start space-x-3 text-[11px]", f.included ? "text-gray-400" : "text-gray-700")}>
            {f.included ? (
              <Check size={14} className={f.highlight ? "text-electric-blue" : "text-emerald"} />
            ) : (
